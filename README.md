...
Format (OKF v0.2) bundle for the Zoho Analytics REST API v2: the API source documents, the hand-written
concept files, the generator scripts, the generated bundle, and the guide that tells an AI agent (or a
human) how to update all of it when the API changes.
...
|---|---|---|
| [`api-docs/`](api-docs/README.md) | **Input.** API reference markdown, OpenAPI specs, SDK samples and the common JSON, as exported from the documentation system. | Yes - place the new or updated export here. |
...
1. Put the new or updated source export in `api-docs/`. The build regenerates the whole bundle from
   whatever is in `api-docs/`, so it must hold the **complete** set of documents, not only the changed
   ones - see [api-docs/README.md](api-docs/README.md).
...
4. Review `git diff bundle/`; commit sources and bundle together.
...
by hand - change `api-docs/`, `handwritten/` or the configuration tables in `tools/build_okf.py`, rebuild,
validate.
